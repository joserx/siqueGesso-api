...
export class RhService {
  ...
  async find(params?) {
    if (params) {
      return await this.rhRepository.find(params);
    }
    return await this.rhRepository.find();
  }

  async findByPage(no) {
    return await this.rhRepository
  ...
  async findOne(id: number) {
    if (id && Number(id)) {
      return await this.rhRepository.findOne(id, {
        relations: ['createdBy', 'avatar', 'anexes', 'falta', 'exame'],
      });
    } else {
  ...
  async disabledData() {
    let disabled = await this.rhRepository.find({
      select: ['status'],
      where: { status: 0 },
    });
    let enabled = await this.rhRepository.find({
      select: ['status'],
      where: { status: 1 },
    });
  ...
  async update(id: number, data: any) {
    let rh = await this.rhRepository.findOne(id, {
      relations: ['createdBy', 'avatar', 'anexes'],
    });
    if (rh) {
      if (id && Number(id)) {
  ...
  async delete(id: number) {
    if (id && Number(id)) {
      let req = await this.rhRepository.findOne(id, {
        relations: ['falta', 'ausencia', 'exame', 'vt'],
      });
